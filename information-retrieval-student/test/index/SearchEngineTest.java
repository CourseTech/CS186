...
public class SearchEngineTest {
	...
	private static final String RATATOUILLE = "Ratatouille (/ËŒrÃ¦tÉ™ËˆtuË�iË�/ rat-É™-TOO-ee; French: [Ê�atatuj]) is a French ProvenÃ§al stewed vegetable dish, originating in Nice, and sometimes referred to as ratatouille niÃ§oise. The word ratatouille derives from the Occitan ratatolha and is related to the French ratouiller and tatouiller, expressive forms of the verb touiller, meaning \"to stir up\". From the late 18th century, in French, it merely indicated a coarse stew. The modern ratatouille - tomatoes as a foundation for sautÃ©ed garlic, onions, zucchini, eggplant, bell peppers, marjoram, fennel and basil, or bay leaf and thyme, or a mix of green herbs like herbes de Provence - does not appear in print until c. 1930.";
	private static final DocumentId RATATOUILLE_ID = new DocumentId("RATATOUILLE");
	private static final String PAELLA = "Paella (Catalan pronunciation: [paËˆeÊŽa] or [pÉ™ËˆeÊŽÉ™], Spanish: [paËˆeÊŽa]; English approximation: /pÉ‘Ë�ËˆeÉªlÉ™, -ËˆeÉªljÉ™, -ËˆeÉªjÉ™, -ËˆÉ›lÉ™, -ËˆjÉ›lÉ™/ or /paÉªËˆÉ›lÉ™/) is a Valencian rice dish with ancient roots that originated in its modern form in the mid-19th century near Albufera lagoon on the east coast of Spain adjacent to the city of Valencia. Many non-Spaniards view paella as Spain's national dish, but most Spaniards consider it to be a regional Valencian dish. Valencians, in turn, regard paella as one of their identifying symbols. Valencian paella is believed to be the original recipe and consists of white rice, green beans (bajoqueta and tavella), meat (chicken and rabbit), white beans (garrofÃ³n), snails, and seasoning such as saffron and rosemary. Another very common but seasonal ingredient is artichokes. Seafood paella replaces meat with seafood and omits beans and green vegetables. Mixed paella is a free-style combination of meat from land animals, seafood, vegetables, and sometimes beans. Most paella chefs use bomba rice due to it being harder to overcook, but Valencians tend to use a slightly stickier (and thus more susceptible to overcooking) variety known as Senia. All types of paellas use olive oil.";
	private static final DocumentId PAELLA_ID = new DocumentId("PAELLA");
	private static final String FRIED_RICE = "Fried rice (Chinese: ç‚’é£¯; pinyin: chÇŽo fÃ n) is a Chinese dish of steamed rice that has been stir-fried in a wok and, usually, mixed with other ingredients, such as eggs, vegetables, and meat, and as such, often served as a complete dish. It is sometimes served as the penultimate dish in Chinese banquets, just before dessert. As a homemade dish, fried rice is typically made with leftover ingredients (including vegetables and/or meat) from other dishes, leading to countless variations, being an economic hodgepodge like it is done with fried noodles or pyttipanna.";
	private static final DocumentId FRIED_RICE_ID = new DocumentId("FRIED_RICE");
	private static final String CHOW_MEIN = "Chow mein (/ËˆtÊƒaÊŠ ËˆmeÉªn/) are stir-fried noodles, the name being the romanization of the Taishanese chÄ�u-mÃ¨ing. The dish is popular throughout the Chinese diaspora and appears on the menus of Chinese restaurants.In American Chinese cuisine, it is a stir-fried dish consisting of noodles, meat (chicken being most common but pork, beef, shrimp or tofu sometimes being substituted), onions and celery. It is often served as a specific dish at westernized Chinese restaurants. Vegetarian or vegan Chow Mein is also common.";
	private static final DocumentId CHOW_MEIN_ID = new DocumentId("CHOW_MEIN");
	private static final String WAT = "Wat, weÌ tâ€™, wot (Amharic: á‹ˆáŒ¥?, IPA: [wÉ™tÊ¼]) or tsebhi (Tigrinya: áŒ¸á‰¥áˆ’?, IPA: [sÊ¼É�bÄ§i]) is an Ethiopian and Eritrean stew or curry that may be prepared with chicken, beef, lamb, a variety of vegetables, spice mixtures such as berbere, and niter kibbeh, a seasoned clarified butter. Several properties distinguish wats from stews of other cultures. Perhaps the most obvious is an unusual cooking technique: the preparation of a wat begins with chopped onions slow cooked, without any fat or oil, in a dry skillet or pot until much of their moisture has been driven away. Fat (usually niter kibbeh) is then added, often in quantities that might seem excessive by modern Western standards, and the onions and other aromatics are sautÃ©ed before the addition of other ingredients. This method causes the onions to break down and thicken the stew. Wat is traditionally eaten with injera, a spongy flat bread made from the millet-like grain known as teff. There are many types of wats. The popular ones are doro wat and siga wat, (Ge'ez: áˆ¥áŒ‹ Å›igÄ�) made with beef.";
	...
	public void testTomatoRelevance() throws Exception {
		addFood();
		assertEquals(Arrays.asList(TOMATO_SAUCE_ID, PIZZA_ID), searchEngine.relevanceLookup("tomato",2));
	}
	...
	public void testMeatRelevanceLookup() throws Exception {
		...
		assertEquals(Arrays.asList(PAELLA_ID, FRIED_RICE_ID, CHOW_MEIN_ID, TOMATO_SAUCE_ID), 
				searchEngine.relevanceLookup("meat",4));
	}
	...
	public void testPastaRelevanceLookup() throws Exception {
		...
		assertEquals(Arrays.asList(SPAGHETTI_ID, TOMATO_SAUCE_ID), 
				searchEngine.relevanceLookup("pasta",2));
	}
	...
	public void testBeefRelevanceLookup() throws Exception {
		...
		assertEquals(Arrays.asList(WAT_ID, CHOW_MEIN_ID), 
				searchEngine.relevanceLookup("beef",2));
	}
	
	@Test
	public void testBeefRelevanceLookupLimit() throws Exception {
		addFood();
		assertEquals(Arrays.asList(WAT_ID), 
				searchEngine.relevanceLookup("beef",1));
	}
	...
	public void testIrrelevantLookup() throws Exception {
		...
		assertEquals(Arrays.asList(),
				searchEngine.relevanceLookup("foobar",1));
	}
}